# Discrete Mathematics Daily Quiz Questions

[![GitHub stars](https://img.shields.io/github/stars/GATE-And-Tech/Discrete-Mathematics-Daily-Quiz-Questions)](https://github.com/GATE-And-Tech/Discrete-Mathematics-Daily-Quiz-Questions/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/GATE-And-Tech/Discrete-Mathematics-Daily-Quiz-Questions)](https://github.com/GATE-And-Tech/Discrete-Mathematics-Daily-Quiz-Questions/network)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

This repository offers a daily dose of quiz questions that delve into various aspects of Discrete Mathematics. Whether you're a student aiming to strengthen your foundational knowledge or an enthusiast eager to solve intriguing problems, this collection is tailored to cater to a spectrum of skill levels.

## 📚 About

This repository is part of the [GATE And Tech initiative](https://gateandtech.in/) to help students prepare for competitive exams like GATE, PGEE, and other technical examinations. Discrete Mathematics is a fundamental topic in computer science and engineering, and this repository provides daily practice questions to master it.

**Topics:** Set Theory, Graph Theory, Combinatorics, Boolean Algebra, Logic, Number Theory

## 🎯 Resources

- Daily quiz questions with solutions
- Practice problems covering set theory, logic, combinatorics, graph theory, and more
- Solutions and explanations for each question

## 🚀 How to Use

1. Clone the repository:
   ```bash
   git clone https://github.com/GATE-And-Tech/Discrete-Mathematics-Daily-Quiz-Questions.git
   ```
2. Navigate to the repository:
   ```bash
   cd Discrete-Mathematics-Daily-Quiz-Questions
   ```
3. Browse through the quiz questions and solutions
4. Practice regularly to improve your discrete mathematics skills

## 🤝 Contributing

Contributions are welcome! Please read our [Contributing Guidelines](CONTRIBUTING.md) before submitting a pull request.

1. Fork the repository
2. Create your feature branch (`git checkout -b feature/AmazingFeature`)
3. Commit your changes (`git commit -m 'Add some AmazingFeature'`)
4. Push to the branch (`git push origin feature/AmazingFeature`)
5. Open a Pull Request

## 📄 License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

## 🌟 Support

If you find this repository helpful, please give it a star! ⭐

## 📞 Connect With Us

- **Telegram Group**: [Join our Telegram Group](https://t.me/gateandtechcse)
- **Telegram Channel**: [Subscribe to our Telegram Channel](https://t.me/gateandtech)
- **GATE And Tech LinkedIn**: [LinkedIn Company Page](https://www.linkedin.com/company/gate-and-tech)
- **Lakshman Patel LinkedIn**: [LinkedIn Profile](https://www.linkedin.com/in/lakshman-patel)
- **YouTube**: [Subscribe to our YouTube Channel](https://www.youtube.com/@gateandtechofficial)
- **X (Twitter)**: [Follow us on X](https://x.com/GATEAndTech)
